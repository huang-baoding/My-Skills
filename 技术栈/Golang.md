...
> mcache的内存来自于mcentral，当在mcache中内存不足时会去mcentral中申请，mcentral中也没有就会去mheap中申请。
...
基本类型所占内存的大小一般为计算机位数的整数倍。结构体中成员的对齐以最大的成员的大小为基准。在Go语言中，编译器会自动帮我们完成内存对齐的过程。
...
> 初始化切片：1~3个；map：1~2个（map没有容量）；channel：1~2个（不能初始化长度）
...
### 1.16  channel注意事项
* 通道属于引用类型，是多个gorotine之间传递数据和同步的重要工具，在使用之前需要用make进行初始化，make之后通道的缓冲容量就被固定了，如果还没有初始化就向通道发送或者接收元素的话会导致当前goroutine永久阻塞。

* 在make的时候可以用第二个参数指定通道的缓冲容量，如果不指定那么就是一个非缓冲通道。缓冲通道有以下机制可以保证并发安全：1.每次只能有一个goroutine可以向通道中发送数据，有多个goroutine需排队发送，如果通道已经满了，试图发送数据的gorotine就会阻塞，直到可以发送数据。2.每次也只能有一个goroutine可以从通道中取数据（随机一个取），取的时候通道为空的话则当前gorotine会阻塞直到可以成功接收数据。非缓冲通道有一些区别：向非缓冲通道中发送或接收数据，当前goroutine都会阻塞，直到有别的goroutine在通道的另一端和它进行握手交换数据。
...
* 基本数据类型：字节型、整型、浮点型、布尔型、复数型、字符串
* 符合数据类型：指针、数组、结构体、切片、Map、通道、函数、接口


* 值类型：字节型、整型、浮点型、布尔型、字符串、数组和结构体
* 引用类型：指针、切片、map、通道channel、接口

> 值类型通常会在栈上分配，引用类型通常会在堆上分配，引用类型存储的是指向底层数据结构的地址。
...
> 并发安全问题指的是多个goroutine在访问同一个资源时，会引发静态条件，产生错误的结果。

* 并发安全（线程安全）
  * channel
  * sync.Mutex 和sync.RWMutex（锁和读写锁）
  * sync.WaitGroup
  * sync.Map
  * sync.Cond
...
* 并发不安全（线程不安全）
  * 基本数据类型：字节型、整型、浮点型、布尔型、字符串、复数型
  * 复合数据类型：数组、结构体、切片、Map

> 需要注意的是，channel在官方的说法中是支持并发安全的，但是不正确使用也会引发错误，例如在多线程的情况下，一个线程去读取或关闭其它线程已经关闭掉的channel都会发生panic。

> 接口的并发安全性取决于实现接口的具体类型。

### 什么是sync.Map

sync.Map 是为并发场景设计的一种高性能、线程安全的映射类型。sync.Map将数据分为读映射和脏映射，数据的添加和修改在脏映射中加锁进行，对脏映射的数据访问次数到达某个值时，会自动将这个数据提升到读映射；读取的时候则是先读取读映射中的数据，不存在再去脏映射中加锁读取。

* 对已经存在于读映射中的数据的修改

  > 在读映射中的数据会先被标记为删除，修改后的新的键值对会被添加到脏映射中，添加的同时修改那个被标记的键值对。

* sync.Map更适合用于读多写少的并发场景：因为对数据的修改都是需要加锁进行的，而且还需要将读映射和脏映射中的数据进行同步。当在写多读少的时候我们可以直接使用锁和Map的方式，省去数据的同步操作。

### Golang的单例模式

> 单例模式的作用是确保无论对象被实例化多少次，全局都只有一个实例存在。

* 饿汉式

```go
//singleton 包在被导入时会自动初始化 instance 实例，使用时通过调用GetSingleton函数即可获得单例对象。
//大多数情况下不推荐使用，单例对象太多会导致程序加载时间过长
package singleton
...
```go
//在第一次调用GetSingleton函数时才会将instance实例化
//但是通过 instance == nil 的判断并不可靠，不能保证并发安全
package singleton
type singleton struct{}
var instance *singleton
func GetSingleton() *singleton {
...
* **闭包函数**
  ...
  * 优点：灵活性、代码复用、状态保持
    ...
    >  闭包函数可以捕获外部作用域的变量，因此非常灵活；闭包函数可以作为参数传递给其他函数，复用代码；闭包函数可以保持外部作用域的变量的状态。
...
* 1.13 版本的defer
  ...
  > 显示循环和隐士循环的defer依然需要使用1.12版本的方式
